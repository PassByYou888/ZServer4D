...
begin
  ...
  ClientTunnel.AsyncConnectP(AddrEdit.Text, 9988, procedure(const cState: Boolean)
    begin
      DoStatus('VM tunnel linked');
      if cState then
        begin
          DoStatus('VM is shaking hands');
...
begin
  ...
  Caption := Format('VM client (%d)... Half open link (%d) link complete (%d)', [TotalCli, connectingCli, ConnectedCli]);
end;
