...
begin
  ...
  for i := 1 to 100000 do
    begin
      ...
      {  The postassemblystream method will submit data to the server using the batchstream mechanism  }
      {  The postassemblystream method compresses and encrypts the data before submitting it  }
      // DataStoreClient.PostAssembleStream('LocalTestDB', de);

      {  The fastpostcompletebuffer method will submit data to the server using the completebuffer mechanism  }
      {  The postassemblystream method will not do any operation on the data before submitting it. If important data is transmitted on the public network, confidential data may be leaked  }
