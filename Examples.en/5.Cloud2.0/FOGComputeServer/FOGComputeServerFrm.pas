...
begin
  DoStatus('%S user is offline', [UserDefineIO.Owner.GetPeerIP]);
...
var
  v: Variant;
begin
  {  After 1.5 seconds, check the offline status of the client  }
  if RecvTunnel.Exists(Sender.Data1) then
    begin
      {  When the client is not offline, the syntax engine is used to calculate the expression result once  }
      v := EvaluateExpressionValue(Sender.DataEng.ReadString(0));
      {  Write the feedback value to the delayed data structure, which is equivalent to outdata  }
      if VarIsNull(v) then
          TPeerClient(Sender.Data1).OutDataFrame.WriteString('Invalid expression')
      else
        begin
          TPeerClient(Sender.Data1).OutDataFrame.WriteString(VarToStr(v));
          FOGComputeServerForm.FDataStoreClient.PostFogComputeInfo('Anonymous user', 'Anonymous user', Sender.DataEng.ReadString(0), VarToStr(v));
        end;
      {  Immediately feed back the calculation results to the client  }
...
begin
  {  Fog calculation simulation  }

  {  Pause feedback  }
  Sender.PauseResultSend;

  {  Delay engine, throw a 1.5 second execution time  }
  {  Delayed throwing is to simulate the background thread or cross service calculation, and continue to feed back the results after the calculation is completed  }
