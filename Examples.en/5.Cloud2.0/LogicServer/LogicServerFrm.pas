...
begin
  ...
  try
      ...
      Format('%S (%s) login', [string(LogicCli.UserConfigFile.Hit['UserInfo', 'Alias']), UserDefineIO.Owner.GetPeerIP]));
  except
  end;
...
begin
  ...
  try
      LogicForm.FDataStoreClient.PostUserLoginInfo(LogicCli.UserID, string(LogicCli.UserConfigFile.Hit['UserInfo', 'Alias']),
      Format('%S (%s) offline', [string(LogicCli.UserConfigFile.Hit['UserInfo', 'Alias']), UserDefineIO.Owner.GetPeerIP]));
  except
  end;
    ...
    begin
      ...
      for i := 0 to Sender.Count - 1 do
        if Sender[i]^.FileLastTime <> 0 then
          begin
            {  User already exists, polling registration  }
            j := Sender.SerialNo + 1;
            repeat
      ...
      {  Complete registration and create basic user information  }
      UserID := Format('guest%.5d', [Sender.SerialNo]);
      te := TSectionTextData.Create;
      te.Hit['UserInfo', 'Alias'] := Format('Tourist%.5D', [Sender.SerialNo]);
      ...
      if RegUser(UserID, UserID, te) then
        ...
        begin
          {  Local registration failed, and then go to remote polling  }
          j := Sender.SerialNo + 1;
          repeat
            UserID := Format('guest%.5d', [j]);
            inc(j);
