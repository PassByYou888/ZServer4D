...
procedure TZDB2_List_HashVariant.ExtractTo(Stream_: TCoreClassStream);
var
  TmpIOHnd: TIOHnd;
...
begin
  ...
  tk := GetTimeTick;
  with TZDB2_List_HashVariant.Create(TZDB2_HashVariant, nil, 5000, M64_1, 64 * 1048576, 200, Cipher_) do
    begin
  ...
  tk := GetTimeTick;
  L := TZDB2_List_HashVariant.Create(TZDB2_HashVariant, nil, 5000, M64_1, 64 * 1048576, 200, Cipher_);
  ...
  tk := GetTimeTick;
  L := TZDB2_List_HashVariant.Create(TZDB2_HashVariant, nil, 5000, M64_2, 64 * 1048576, 200, Cipher_);
  for i := 0 to L.Count - 1 do
    begin
      if L[i].Data['a'] <> 'abcdefg' then
          DoStatus('%s - test error.', [L.ClassName]);
