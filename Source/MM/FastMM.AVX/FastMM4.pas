...
const
  {XCR0[2:1] = 1b(XMM state and YMM state are enabled by OS).}
  cXcrXmmAndYmmMask = (4-1) shl 1;

{$ifdef EnableAVX512}
const
  {XCR0[7:5] = 11b(OPMASK state, upper 256-bit of ZMM0-ZMM15 and ZMM16-ZMM31 state are enabled by OS).}
...
2) Issue XGETBV and verify that XCR0[2:1] = 1b(XMM state and YMM state are enabled by OS).
...
ENDQUOTE}
      ...
      {$ifdef EnableAVX}
      if
         {verify that XCR0[2:1] = 1b(XMM state and YMM state are enabled by OS).}
