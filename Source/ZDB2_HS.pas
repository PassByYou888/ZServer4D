{ ****************************************************************************** }
{ * ZDB 2.0 automated fragment for Hash Text support, create by.qq600585       * }
{ * https://zpascal.net                                                        * }
...
type
  ...
  TZDB2_List_HashString_Decl = {$IFDEF FPC}specialize {$ENDIF FPC} TGenericsList<TZDB2_HashString>;

  TOnCreate_ZDB2_HashString = procedure(Sender: TZDB2_List_HashString; Obj: TZDB2_HashString) of object;

  TZDB2_List_HashString = class(TZDB2_List_HashString_Decl)
  private type
...
procedure TZDB2_List_HashString.Delete(Index: Integer; RemoveData_: Boolean);
begin
...
procedure TZDB2_List_HashString.Clear(RemoveData_: Boolean);
var
  i: Integer;
begin
...
procedure TZDB2_List_HashString.ExtractTo(Stream_: TCoreClassStream);
var
  TmpIOHnd: TIOHnd;
...
begin
  ...
  tk := GetTimeTick;
  with TZDB2_List_HashString.Create(TZDB2_HashString, nil, 5000, M64_1, 64 * 1048576, 200, Cipher_) do
    begin
      ...
      DoStatus('build %d of HashString,time:%dms', [Count, GetTimeTick - tk]);
      Free;
    end;

  tk := GetTimeTick;
  L := TZDB2_List_HashString.Create(TZDB2_HashString, nil, 5000, M64_1, 64 * 1048576, 200, Cipher_);
  for i := 0 to L.Count - 1 do
    begin
      if L[i].Data['a'] <> 'abcdefg' then
          DoStatus('%s - test error.', [L.ClassName]);
    end;
  DoStatus('load %d of HashString,time:%dms', [L.Count, GetTimeTick - tk]);
  L.ExtractTo(M64_2);
  L.Free;

  tk := GetTimeTick;
  L := TZDB2_List_HashString.Create(TZDB2_HashString, nil, 5000, M64_2, 64 * 1048576, 200, Cipher_);
  for i := 0 to L.Count - 1 do
    begin
      if L[i].Data['a'] <> 'abcdefg' then
          DoStatus('%s - test error.', [L.ClassName]);
    end;
  DoStatus('load %d extract stream of HashString,time:%dms', [L.Count, GetTimeTick - tk]);
  L.Free;
