...
begin
  ...
  for i := 0 to lst.Count - 1 do
    begin
      client.GetPublicFileInfo(lst[i], nil, nil,
  ...
  hashSiz.Progress(procedure(Sender: THashVariantList; Name: PSystemString; const v: Variant)
    begin
      client.GetPublicFileMD5(Name^, 0, v, nil, nil,
        procedure(const UserData: Pointer; const UserObject: TCoreClassObject;
  ...
  DoStatus('远程文件列表:');
  hashMD5.Progress(procedure(Sender: THashStringList; Name: PSystemString; const v: SystemString)
    begin
...
begin
  // 先从远程下载完整文件
  client.GetPublicFile('ADRestoreServer.exe', umlCurrentPath, nil, nil,
    ...
    begin
      ...
      // 获取远程文件截断的md5
      client.GetPublicFileMD5('ADRestoreServer.exe', 0, 512, nil, nil,
        procedure(const UserData: Pointer; const UserObject: TCoreClassObject;
          const fileName: SystemString; const StartPos, EndPos: Int64; const MD5: UnicodeMixedLib.TMD5)
        begin
          ...
          // 获取远程文件完整的md5
          client.GetPublicFileMD5('ADRestoreServer.exe', 0, 0, nil, nil,
            procedure(const UserData: Pointer; const UserObject: TCoreClassObject;
              const fileName: SystemString; const StartPos, EndPos: Int64; const MD5: UnicodeMixedLib.TMD5)
            begin
              DoStatus('remote MD5:%s', [umlMD52String(MD5).Text]);
              // 从远程断点续传下载
              client.GetPublicFile('ADRestoreServer.exe', 512, umlCurrentPath, nil, nil,
                procedure(const UserData: Pointer; const UserObject: TCoreClassObject; stream: TCoreClassStream; const fileName: SystemString)
                begin
                  ...
                  // 现在我们来演示断点续传的上传功能
                  client.ProgressEngine.PostExecute(1.0, procedure(Sender: TNPostExecute)
                    var
                      m64: TMemoryStream64;
                    begin
                      m64 := TMemoryStream64.Create;
                      ...
                      client.SendTunnel.Wait(10 * 1000,
                        procedure(const State: Boolean)
                        begin
                          // 在这里触发事件，表示上传已经完成了
                          client.GetPrivateFileMD5('testUpload.dat', '', 0, 0, nil, nil,
                            procedure(const UserData: Pointer; const UserObject: TCoreClassObject;
                              const fileName: SystemString; const StartPos, EndPos: Int64; const MD5: UnicodeMixedLib.TMD5)
                            var
