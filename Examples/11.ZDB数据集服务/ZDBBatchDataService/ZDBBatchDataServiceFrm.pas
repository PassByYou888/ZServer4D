...
type
  TMyDataStoreService = class(TDataStoreService)
  protected
...
procedure TMyDataStoreService.UserLinkSuccess(UserDefineIO: TPeerClientUserDefineForRecvTunnel);
begin
  DoStatus('用户 %s 建立交互链接成功', [UserDefineIO.UserID]);
  inherited;
end;

procedure TMyDataStoreService.UserLoginSuccess(UserDefineIO: TPeerClientUserDefineForRecvTunnel);
begin
  DoStatus('用户 %s 登录成功', [UserDefineIO.UserID]);
  inherited;
end;

procedure TMyDataStoreService.UserOut(UserDefineIO: TPeerClientUserDefineForRecvTunnel);
begin
...
begin
  ...
  DBService.CanSaveUserInfo := False;
  DBService.CanRegisterNewUser := False;
  ...
  DBService.RegUser('test1', 'test1', nil);
  DBService.RegUser('test2', 'test2', nil);
